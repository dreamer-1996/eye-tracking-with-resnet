...
from gaze_estimation import (GazeEstimationMethod, create_dataloader,
                             create_model)
from gaze_estimation.utils import load_config, compute_angle_error, save_config,compute_linear_error
...
def test(model, test_loader, config):
    ...
    gts = []
    with torch.no_grad():
        for images, poses, targets in tqdm.tqdm(test_loader):
    ...
    gts = torch.cat(gts)
    angle_error = float(compute_linear_error(predictions, gts).mean())
...
def main():
    ...
    print(f'The mean Linear error : {angle_error:.2f}')
